...
def my_class_balanced_cross_entropy_loss(output, label):
    ...
    """
    label_gr0 = tf.cast(tf.greater(label,0),tf.float32)
    label_gr255 = tf.cast(tf.greater(label,200),tf.float32)
    num_labels_pos = tf.reduce_sum(label_gr0-label_gr255)
    num_labels_neg = tf.reduce_sum(1-label_gr0)
    ...
    loss_pos = tf.reduce_sum(-tf.multiply(label_gr0-label_gr255, loss_val))
    loss_neg = tf.reduce_sum(-tf.multiply(1-label_gr0, loss_val))
...
def myloss(net,label):
    [b,_,_,_]  = np.shape(net)
    a = np.array([[1,2,3],[4,5,6]])
    a = tf.constant(a,dtype=tf.float32)
    for i in range(b):
        net = net[i,:,:,:]
    net = tf.log(net)
    loss = tf.reduce_sum(net)+tf.reduce_sum(a)
    return loss
...
def _train(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step, display_step,
           ...
           test_image_path=None, ckpt_name="osvos",stopbp=False):
    """Train
    Args:
    ...
    with slim.arg_scope(osvos_arg_scope()):
            net,net_5,end_points = osvos(input_image)
    ...
    with tf.name_scope('losses'):
        if supervison == 1 or supervison == 2:
            dsn_2_loss = my_class_balanced_cross_entropy_loss(end_points['osvos/score-dsn_2-cr'], input_label)
            tf.summary.scalar('dsn_2_loss', dsn_2_loss)
            dsn_3_loss = my_class_balanced_cross_entropy_loss(end_points['osvos/score-dsn_3-cr'], input_label)
            tf.summary.scalar('dsn_3_loss', dsn_3_loss)
            dsn_4_loss = my_class_balanced_cross_entropy_loss(end_points['osvos/score-dsn_4-cr'], input_label)
            tf.summary.scalar('dsn_4_loss', dsn_4_loss)
            dsn_5_loss = my_class_balanced_cross_entropy_loss(end_points['osvos/score-dsn_5-cr'], input_label)
            tf.summary.scalar('dsn_5_loss', dsn_5_loss)
        # my = myloss(net_5)
        main_loss = my_class_balanced_cross_entropy_loss(net, input_label)
    ...
    with tf.Session(config=config) as sess:
        ...
        while step < max_training_iters + 1:
            ...
            for _ in range(0, iter_mean_grad):
                ...
                run_res ,net5= sess.run([[total_loss, merged_summary_op] + grad_accumulator_ops,net_5],
...
def train_refinetune(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step,
                   display_step, global_step, iter_mean_grad=1, batch_size=1, momentum=0.9, resume_training=False,
                   config=None, test_image_path=None, ckpt_name="osvos"):
    """Finetune osvos
    Args:
    ...
    _train(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step, display_step,
           global_step, iter_mean_grad, batch_size, momentum, resume_training, config, finetune, test_image_path,
           ckpt_name,stopbp=True)


def segmentation(dataset, checkpoint_file, result_path, config=None):
    """segmentation one sequence
    Args:
    dataset: Reference to a Dataset object instance
    ...
    op = g1.get_tensor_by_name('osvos/conv5/conv5_2/weights:0')
    ...
    with tf.Session(config=config) as sess:
        ...
        for frame in range(0, dataset.get_test_size()):
            ...
            res_np = res.astype(np.float32)[0, :, :, 0]   # gai lv da yu 0.63 de ren wei shi mu b
            res_np = res.astype(np.float32)[0, :, :, 0] > 162.0/255.0#gai lv da yu 0.63 de ren wei shi mu biao
